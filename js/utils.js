function convert2d(x, y, z) {
    return {
        x: center_x + (x * FOCAL_LENGTH) / (z * 0.8 + FOCAL_LENGTH),
        y: center_y - (y * FOCAL_LENGTH) / (z * 0.8 + FOCAL_LENGTH)
    };
}
...
function getJSON(url, data, success, error, anyway) {
